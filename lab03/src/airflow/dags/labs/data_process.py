...
def _split_features(df: pd.DataFrame) -> tuple:
    ...
    cat_columns = [col for col, dtype in zip(column_names, df.dtypes) if dtype == "object"]
    num_columns = [col for col, dtype in zip(column_names, df.dtypes) if dtype in ["int64", "float64"]]
